...
mod tests {
    ...
    fn test_store() -> Option<AccountStore> {
        dotenvy::dotenv().ok();
        env::set_var("INGESTION_QUEUE_TLS_ALLOW_INVALID_CERTS", "true");
        AccountStore::from_env().ok()
    }

    #[test]
    fn create_and_get_account() {
        let Some(store) = test_store() else {
            eprintln!("Skipping test: SUPABASE_DB_URL not set");
            return;
        };

        let auth_user_id = Uuid::new_v4();
        let account = store.create_account(auth_user_id).expect("create account");
        assert_eq!(account.auth_user_id, auth_user_id);

        let fetched = store.get_account_by_auth_user(auth_user_id).expect("get account");
        assert!(fetched.is_some());
        assert_eq!(fetched.unwrap().id, account.id);
    }

    #[test]
    fn create_and_verify_identifier() {
        let Some(store) = test_store() else {
            eprintln!("Skipping test: SUPABASE_DB_URL not set");
            return;
        };
        ...
        let identifier = store
            .create_identifier(account.id, "phone", "+14155550100")
            .expect("create identifier");
        assert!(!identifier.verified);

        store
            .verify_identifier(account.id, "phone", "+14155550100")
            .expect("verify");

        let found = store
            .get_account_by_identifier("phone", "+14155550100")
            .expect("lookup");
