...
fn try_quick_response_telegram(
    config: &ServiceConfig,
    user_store: &UserStore,
    message_router: &MessageRouter,
    runtime: &tokio::runtime::Handle,
...
fn append_telegram_message(
    ...
    fs::write(incoming_dir.join(&json_filename), raw_payload)?;
    ...
    if let Some(ref text) = message.text_body {
        ...
        fs::write(incoming_dir.join(&txt_filename), content)?;
    }
