//! Inbound adapter for Google Slides comments.
...
/// Adapter for polling Google Slides comments.
#[derive(Debug, Clone)]
pub struct GoogleSlidesInboundAdapter {
    comments_client: GoogleCommentsClient,
    auth: GoogleAuth,
}

impl GoogleSlidesInboundAdapter {
    ...
    /// List all presentations shared with the authenticated user.
    ...
    pub fn actionable_to_inbound_message_with_owner(
        ...
        let thread_id = format!("{}:{}", presentation_id, actionable.comment.id);
        ...
        InboundMessage {
            ...
            subject: Some(format!("Comment on: {}", presentation_name)),
    ...
    /// Get presentation structure using Slides API.
...
impl InboundAdapter for GoogleSlidesInboundAdapter {
    fn parse(&self, _raw_payload: &[u8]) -> Result<InboundMessage, AdapterError> {
        Err(AdapterError::ParseError(
            "GoogleSlidesInboundAdapter is poll-based; use actionable_to_inbound_message instead"
                .to_string(),
        ))
    }
