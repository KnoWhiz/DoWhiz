...
- Runtime `.env` must use unprefixed keys only (no `STAGING_*` / `PROD_*`).
...
| IT-RUN-14 | run_task_skips_yolo_without_bypass | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Keeps sandboxed execution without bypass flag | Real codex behavior | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-15 | run_task_uses_yolo_with_bypass | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | `CODEX_BYPASS_SANDBOX` -> `--yolo` wiring | Real codex behavior | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-16 | run_task_uses_danger_sandbox_with_bypass | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Bypass mode writes dangerous sandbox config | Real codex behavior | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-17 | run_task_passes_add_dir_for_gh_config | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | GitHub config dir passthrough to Codex | Actual gh auth state | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-18 | run_task_reports_turn_aborted_as_codex_failure | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Detects runtime failure markers in successful process exit | All Codex failure variants | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-19 | run_task_times_out_with_codex | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Timeout handling for Codex runner | Slow subprocess edge cases | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-20 | run_task_times_out_with_claude | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Timeout handling for Claude runner | Slow subprocess edge cases | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-21 | run_task_maps_x402_env_from_dotenv | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | x402 payment env propagation from dotenv | Real payment backend | AUTO | cargo test -p run_task_module --test run_task_tests |
| IT-RUN-22 | run_task_maps_employee_prefixed_x402_env_from_dotenv | run_task_module::run_task | DoWhiz_service/run_task_module/tests/run_task_tests.rs | Employee-prefixed x402 env precedence | Real payment backend | AUTO | cargo test -p run_task_module --test run_task_tests |
...
| IT-SCH-02 | one_shot_persists_across_restarts | scheduler_module::Scheduler | DoWhiz_service/scheduler_module/tests/scheduler_basic.rs | Mongo-backed scheduler persistence across reload | Concurrency | AUTO | cargo test -p scheduler_module --test scheduler_basic |
...
| IT-SCH-14 | run_task_followups_persist_to_mongo_scoped_store | Scheduler follow-ups | DoWhiz_service/scheduler_module/tests/scheduler_followups.rs | Follow-up persisted across scheduler reload | Multiple follow-ups | AUTO | cargo test -p scheduler_module --test scheduler_followups |
...
| IT-SCH-22 | email_flow_injects_x402_env | process_inbound_payload + run_task | DoWhiz_service/scheduler_module/tests/github_env_e2e.rs | x402 env injection from dotenv | Real payment backend | AUTO | cargo test -p scheduler_module --test github_env_e2e |
| IT-SCH-23 | email_flow_injects_employee_prefixed_x402_env | process_inbound_payload + run_task | DoWhiz_service/scheduler_module/tests/github_env_e2e.rs | Employee-prefixed x402 env injection | Real payment backend | AUTO | cargo test -p scheduler_module --test github_env_e2e |
| IT-SCH-24 | scheduler_run_task_injects_x402_env_from_dotenv | scheduler run_task env bridge | DoWhiz_service/scheduler_module/tests/scheduler_x402_env_e2e.rs | Scheduler -> run_task x402 env propagation | Real payment backend | AUTO | cargo test -p scheduler_module --test scheduler_x402_env_e2e |
...
| LIVE-SCH-11 | rust_service_real_email_end_to_end | run_server + gateway + Postmark | DoWhiz_service/scheduler_module/tests/service_real_email.rs | Real inbound/outbound email flow | Cost and external variance | LIVE | `RUST_SERVICE_LIVE_TEST=1` + Postmark/ngrok (`POSTMARK_SMTP_PORT` optional, set `2525` when port 25 is blocked) |
...
| LIVE-SCH-13 | billing_e2e_balance_and_payments | account/billing store | DoWhiz_service/scheduler_module/tests/billing_e2e.rs | Balance math, grace-period gating, payment idempotency | Full Stripe webhook flow | LIVE | SUPABASE_DB_URL + TEST_ACCOUNT_ID + cargo test -p scheduler_module --test billing_e2e |
...
- Current staging inbound webhook URL in deployment docs: https://shayne-laminar-lillian.ngrok-free.dev/postmark/inbound
